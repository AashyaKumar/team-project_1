...
$( "td" ).mousedown(function(){
    ...
    var coinCellIndex = (coinCell[0] * (totalCols)) + coinCell[1];
...
$( "td" ).mouseenter(function() {
    if (!createWalls && !movingStart && !movingEnd && !movingCoin){ return; }
    var index = $( "td" ).index( this );
    var startCellIndex = (startCell[0] * (totalCols)) + startCell[1];
    var endCellIndex = (endCell[0] * (totalCols)) + endCell[1];
    var coinCellIndex = (coinCell[0] * (totalCols)) + coinCell[1];
    if (!inProgress){
        ...
        if (movingStart && index != endCellIndex && index != coinCellIndex) {
            moveStartOrEnd(startCellIndex, index, "start");
        } else if (movingEnd && index != startCellIndex && index != coinCellIndex ) {
            moveStartOrEnd(endCellIndex, index,"end");
        }else if (movingCoin && index != startCellIndex && index != endCellIndex ) {
            moveStartOrEnd(endCellIndex, index, "coin");
        }
...
$( "td" ).click(function() {
    ...
    var endCellIndex = (endCell[0] * (totalCols)) + endCell[1];
    var coinCellIndex = (coinCell[0] * (totalCols)) + coinCell[1];
    if ((inProgress == false) && !(index == startCellIndex) && !(index == endCellIndex) && !(index == coinCellIndex)){
        if ( justFinished ){ 
            clearBoard( keepWalls = true );
            justFinished = false;
        }
...
function moveCoin(prevIndex, newIndex){
    ...
    var newCoinX = Math.floor(newIndex / totalRows);
    var newCoinY = Math.floor(newIndex / totalCols);
    startCell = [newStartX, newStartY];
    return;
}
...
function executeAlgo(){
    if (algorithm == "Depth-First Search (DFS)"){
        ...
        var pathFound = BFS(startCell,coinCell);
...
async function animateCells(){
    ...
    for (var i = 0; i < cellsToAnimate.length; i++){
        ...
        if (num == startCellIndex || num == coinCellIndex || num==endCellIndex){ continue; }

        var cell = cells[num];
        var colorClass = cellsToAnimate[i][1];
        if(colorClass == "visited" || colorClass == "searching"){continue;}
